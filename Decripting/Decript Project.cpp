/**
*
* Solution to course project # 2
* Introduction to programming course
* Faculty of Mathematics and Informatics of Sofia University
* Winter semester 2022/2023
*
* @author Mihail Yordanov
* @idnumber 7MI0600198 @compiler VC
*
* Decripting file
*
*/
#include <iostream>
#include <cstring>
#include <fstream>
#include <sstream>
#include <windows.h>


unsigned char s_Box[256] =
{
	
};

// Used in KeyExpansion
unsigned char rcon[256] = {
	
};

// Decryption: Inverse Rijndael S-box
unsigned char inv_s[256] =
{
	
};

// Decryption: Multiply by 9 for InverseMixColumns
unsigned char mul9[256] =
{
	
};

// Decryption: Multiply by 11 for InverseMixColumns
unsigned char mul11[256] =
{
	
};

// Decryption: Multiply by 13 for InverseMixColumns
unsigned char mul13[256] =
{
	
};

// Decryption: Multiply by 14 for InverseMixColumns
unsigned char mul14[256] =
{
	
};

// Auxiliary function for KeyExpansion
void KeyExpansionCore(unsigned char* tmpCore, unsigned char index)
{
	// Rotate left by one byte: shift left 
	unsigned char swapVar = tmpCore[0];
	tmpCore[0] = tmpCore[1];
	tmpCore[1] = tmpCore[2];
	tmpCore[2] = tmpCore[3];
	tmpCore[3] = swapVar;

	// S-box 4 bytes 
	tmpCore[0] = s_Box[tmpCore[0]];
	tmpCore[1] = s_Box[tmpCore[1]];
	tmpCore[2] = s_Box[tmpCore[2]];
	tmpCore[3] = s_Box[tmpCore[3]];

	// RCon
	tmpCore[0] = tmpCore[0] ^ rcon[index];
}

/* The main KeyExpansion function
 * Generates additional keys using the original key
 * Total of 11 128-bit keys generated, including the original
 * Keys are stored one after the other in expandedKeys
 */
void KeyExpansion(unsigned char inputKey[16], unsigned char expandedKeys[176]) {
	// The first 128 bits are the original key
	for (int i = 0; i < 16; i++) {
		expandedKeys[i] = inputKey[i];
	}

	int bytesGenerated = 16; // Bytes we've generated so far
	int rconIteration = 1; // Keeps track of rcon value
	unsigned char tmpCore[4]; // Temp storage for core

	while (bytesGenerated < 176) {
		/* Read 4 bytes for the core
		* They are the previously generated 4 bytes
		* Initially, these will be the final 4 bytes of the original key
		*/
		for (int i = 0; i < 4; i++) {
			tmpCore[i] = expandedKeys[i + bytesGenerated - 4];
		}

		// Perform the core once for each 16 byte key
		if (bytesGenerated % 16 == 0) {
			KeyExpansionCore(tmpCore, rconIteration++);
		}

		for (unsigned char a = 0; a < 4; a++) {
			expandedKeys[bytesGenerated] = expandedKeys[bytesGenerated - 16] ^ tmpCore[a];
			bytesGenerated++;
		}

	}
}

/* Used in Round() and serves as the final round during decryption
 * SubRoundKey is simply an XOR of a 128-bit block with the 128-bit key.
 * So basically does the same as AddRoundKey in the encryption
 */
void SubRoundKey(unsigned char* state, unsigned char* roundKey)
{
	for (int i = 0; i < 16; i++) {
		state[i] = state[i] ^ roundKey[i];
	}
}

/* InverseMixColumns uses mul9, mul11, mul13, mul14 look-up tables
 * Unmixes the columns by reversing the effect of MixColumns in encryption
 */
void InverseMixColumns(unsigned char* state) {
	unsigned char pomState[16];

	pomState[0] = (unsigned char)mul14[state[0]] ^ mul11[state[1]] ^ mul13[state[2]] ^ mul9[state[3]];
	pomState[1] = (unsigned char)mul9[state[0]] ^ mul14[state[1]] ^ mul11[state[2]] ^ mul13[state[3]];
	pomState[2] = (unsigned char)mul13[state[0]] ^ mul9[state[1]] ^ mul14[state[2]] ^ mul11[state[3]];
	pomState[3] = (unsigned char)mul11[state[0]] ^ mul13[state[1]] ^ mul9[state[2]] ^ mul14[state[3]];

	pomState[4] = (unsigned char)mul14[state[4]] ^ mul11[state[5]] ^ mul13[state[6]] ^ mul9[state[7]];
	pomState[5] = (unsigned char)mul9[state[4]] ^ mul14[state[5]] ^ mul11[state[6]] ^ mul13[state[7]];
	pomState[6] = (unsigned char)mul13[state[4]] ^ mul9[state[5]] ^ mul14[state[6]] ^ mul11[state[7]];
	pomState[7] = (unsigned char)mul11[state[4]] ^ mul13[state[5]] ^ mul9[state[6]] ^ mul14[state[7]];

	pomState[8] = (unsigned char)mul14[state[8]] ^ mul11[state[9]] ^ mul13[state[10]] ^ mul9[state[11]];
	pomState[9] = (unsigned char)mul9[state[8]] ^ mul14[state[9]] ^ mul11[state[10]] ^ mul13[state[11]];
	pomState[10] = (unsigned char)mul13[state[8]] ^ mul9[state[9]] ^ mul14[state[10]] ^ mul11[state[11]];
	pomState[11] = (unsigned char)mul11[state[8]] ^ mul13[state[9]] ^ mul9[state[10]] ^ mul14[state[11]];

	pomState[12] = (unsigned char)mul14[state[12]] ^ mul11[state[13]] ^ mul13[state[14]] ^ mul9[state[15]];
	pomState[13] = (unsigned char)mul9[state[12]] ^ mul14[state[13]] ^ mul11[state[14]] ^ mul13[state[15]];
	pomState[14] = (unsigned char)mul13[state[12]] ^ mul9[state[13]] ^ mul14[state[14]] ^ mul11[state[15]];
	pomState[15] = (unsigned char)mul11[state[12]] ^ mul13[state[13]] ^ mul9[state[14]] ^ mul14[state[15]];

	for (int i = 0; i < 16; i++)
	{
		state[i] = pomState[i];
	}
}

// Shifts rows right (rather than left) for decryption
void ShiftRows(unsigned char* state) {
	unsigned char tmp[16];

	/* Column 1 */
	tmp[0] = state[0];
	tmp[1] = state[13];
	tmp[2] = state[10];
	tmp[3] = state[7];

	/* Column 2 */
	tmp[4] = state[4];
	tmp[5] = state[1];
	tmp[6] = state[14];
	tmp[7] = state[11];

	/* Column 3 */
	tmp[8] = state[8];
	tmp[9] = state[5];
	tmp[10] = state[2];
	tmp[11] = state[15];

	/* Column 4 */
	tmp[12] = state[12];
	tmp[13] = state[9];
	tmp[14] = state[6];
	tmp[15] = state[3];

	for (int i = 0; i < 16; i++) {
		state[i] = tmp[i];
	}
}

/* Perform substitution to each of the 16 bytes
 * Uses inverse S-box as lookup table
 */
void SubBytes(unsigned char* state)
{
	for (int i = 0; i < 16; i++) // Perform substitution to each of the 16 bytes
	{
		state[i] = inv_s[state[i]];
	}
}

/* Each round operates on 128 bits at a time
 * The number of rounds is defined in AESDecrypt()
 * Not surprisingly, the steps are the encryption steps but reversed
 */
void Round(unsigned char* state, unsigned char* key)
{
	SubRoundKey(state, key);
	InverseMixColumns(state);
	ShiftRows(state);
	SubBytes(state);
}

// Same as Round() but no InverseMixColumns
void InitialRound(unsigned char* state, unsigned char* key)
{
	SubRoundKey(state, key);
	ShiftRows(state);
	SubBytes(state);
}

/* The AES decryption function
 * Organizes all the decryption steps into one function
 */
void AESDecrypt(unsigned char* encryptedMessage, unsigned char* expandedKey, unsigned char* decryptedMessage)
{
	unsigned char state[16]; // Stores the first 16 bytes of encrypted message

	for (int i = 0; i < 16; i++) {
		state[i] = encryptedMessage[i];
	}

	InitialRound(state, expandedKey + 160);

	int numberOfRounds = 9;

	for (int i = 8; i >= 0; i--) {
		Round(state, expandedKey + (16 * (i + 1)));
	}

	SubRoundKey(state, expandedKey); // Final round

	// Copy decrypted state to buffer
	for (int i = 0; i < 16; i++) {
		decryptedMessage[i] = state[i];
	}
}

int StringLenght(char* str)
{
	int i = 0;
	while (str[i] != NULL)
	{
		i++;
	}
	return i;
}

int main()
{
	//interface
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(hConsole, 11);
	std::cout << "========================================================================================================================" << std::endl;
	std::cout << "     ___      _____     _____              __          __       _                __           ________         ____     " << std::endl;
	std::cout << "    /   |   /  _   \\   /  _  \\            |  |        |__|    _| |_             /  \\         |   _____|       / _  \\" << std::endl;
	std::cout << "   / /| |  |  |  \\  |  | / \\  |           |  |         ___   |_   _|           / /\\ \\        |  |            / / /_/" << std::endl;
	std::cout << "  /_/ | |   \\_\\   / /  \\ \\_/ /    _____   |  |        |  |     | |            / /__\\ \\       |  |_____      | |   " << std::endl;
	std::cout << "      | |        / /    | _  |   |_____|  |  |___     |  |     | |           /  ____  \\      |   _____|      \\ \\     " << std::endl;
	std::cout << "      | |       / /    / / \\ \\            |  ___ \\    |  |     | |          /  /    \\  \\     |  |          __ | |  " << std::endl;
	std::cout << "      | |      / /__   | \\_/  |           | |___| |   |  |     | |_        /  /      \\  \\    |  |_____    / /_/ /    " << std::endl;
	std::cout << "      |_|     |_____|  \\_____/            |______/    |__|     \\__/       /__/        \\__\\   |________|   \\____/   " << std::endl;
	std::cout << "" << std::endl;
	std::cout << "========================================================================================================================" << std::endl;
	std::cout << "" << std::endl;
	std::cout << "" << std::endl;
	SetConsoleTextAttribute(hConsole, 7);

	// Read in the message from message.aes
	std::string str;
	std::ifstream infile;
	infile.open("C:\\Users\\User\\source\\repos\\Encrypt Project\\Encrypt Project\\message.aes", std::ios::in | std::ios::binary);
	if (infile.is_open())
	{
		std::getline(infile, str); // The first line of file is the message
		std::cout << "Read in encrypted message from message.aes" << std::endl;
		infile.close();
	}
	else
	{
		std::cout << "Unable to open file1";
	}

	//Makeing to char
	unsigned char* encryptedMessage = new unsigned char[str.size() + 1];
	int ind = 0;
	while (encryptedMessage[ind] != NULL && str[ind] != NULL)
	{
		encryptedMessage[ind] = str[ind];
		ind++;
	}

	//Get String Lenght
	std::string stringSize;
	std::ifstream textSize;
	textSize.open("C:\\Users\\User\\source\\repos\\Encrypt Project\\Encrypt Project\\Text_Size.txt", std::ios::in | std::ios::binary);
	if (textSize.is_open())
	{
		std::getline(textSize, stringSize); // The first line of file is the message
		std::cout << "Read in stringSize message from TextSize" << std::endl;
		textSize.close();
	}
	else
	{
		std::cout << "Unable to open file3" << std::endl;
	}
	int lenghtOfText = std::stoi(stringSize);
	std::cout << std::endl;

	//Get encryptedMessage Lenght
	int n = strlen((const char*)encryptedMessage);

	unsigned char* unsignedEncryptedMessage = new unsigned char[n];
	int index = 0;
	while (encryptedMessage[index] != 0)
	{
		unsignedEncryptedMessage[index] = (unsigned char)encryptedMessage[index];
		index++;
	}

	// Read in the key
	std::string keystr;
	std::ifstream keyfile;
	keyfile.open("TheKey.txt", std::ios::in | std::ios::binary);
	if (keyfile.is_open())
	{
		std::getline(keyfile, keystr); // The first line of file should be the key
		std::cout << "Read in the 128-bit key from keyfile" << std::endl;
		keyfile.close();
	}
	else
	{
		std::cout << "Unable to open file2";
	}

	//Make Key Ten Number System
	std::istringstream hex_chars_stream(keystr);
	unsigned char key[16];
	int i = 0;
	unsigned int number;
	while (hex_chars_stream >> std::hex >> number)
	{
		key[i] = number;
		i++;
	}

	//Key Expansion
	unsigned char expandedKey[176];
	KeyExpansion(key, expandedKey);

	//Decript Message
	int encryptedMessageLenght = strlen((const char*)unsignedEncryptedMessage);
	unsigned char* decryptedMessage = new unsigned char[encryptedMessageLenght];
	for (int i = 0; i < encryptedMessageLenght; i = i + 16)
	{
		AESDecrypt(unsignedEncryptedMessage + i, expandedKey, decryptedMessage + i);
	}

	std::cout << std::endl;
	std::cout << "Decrypted message in hex:" << std::endl;
	for (int i = 0; i < encryptedMessageLenght; i++)
	{
		std::cout << std::hex << (int)decryptedMessage[i];
		std::cout << " ";
	}
	std::cout << std::endl;
	std::cout << std::endl;

	std::cout << "Decrypted message: " << std::endl;
	for (int i = 0; i < lenghtOfText; i++)
	{
		std::cout << decryptedMessage[i];
	}
	std::cout << std::endl;



	// Write the encrypted string out to file "Save.txt"
	std::ofstream outfile;
	outfile.open("Saved_Text.txt", std::ios::out | std::ios::binary);
	if (outfile.is_open())
	{
		outfile << decryptedMessage;
		outfile.close();
		std::cout << "Wrote encrypted message to file Saved_Text" << std::endl;
	}
	else
	{
		std::cout << "Unable to open file3";
		return -1;
	}


	//Free memory
	delete[] unsignedEncryptedMessage;

	return 0;
}

